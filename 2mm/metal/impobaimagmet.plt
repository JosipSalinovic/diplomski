...
set output 'usporedba-imagfreemetal.png'
...
#set xtics 0,1,25
set multiplot
  set key top left #POSTAVLJA LEGENDU GORE DESNO
  ...
  set mxtics 2
  set xrange[700:1200]
  set title "Imaginarna impedancija antene iznad metalne ploče"
    set xlabel "{/Times-New-Roman-Italic f } [MHz] "
    set ylabel " {/Times-New-Roman-Italic X }[{/Symbol W}]"
    #set label "500 šetača" at graph 0.05, graph 0.95
    #set label "VMC (b=1.2)" at graph 0.05, graph 0.87
    plot \
         "D900_M.txt" u ($2*1000):9 w l ls 1  ti "Eksperiment", \
		 "metalimagnec.txt" u 1:2 w l ls 3 ti "Simulator: 4NEC2",\
		 "suzimagmm.txt" u ($1):($2) w l ls 4 ti "Simulator: Suzana"
